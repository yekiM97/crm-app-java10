	...
	<div class="d-flex justify-content-between">
		...
		<div id="admin-wrapper">
			...
			<section id="admin-content" class="p-3">
				...
				<div class="row">
					<div class="col-md-8">
						<a href="<%=request.getContextPath()%>/taskCategory/add"
				...
				<table class="table table-bordered table-hover mt-3">
					...
					<tbody>
						<c:forEach var="taskCategory" items="${taskCategory}" varStatus="index">

							<tr>
								<td>${index.count}</td>
								<td>${taskCategory.name}</td>
								<td>${taskCategory.startDate}</td>
								<td>${taskCategory.endDate}</td>
								<td>${taskCategory.userName}</td>
								<td>${taskCategory.taskName}</td>
								<td>${taskCategory.statusName}</td>
								<td><a
									href="<%= request.getContextPath() %>/taskCategory/edit?id=${taskCategory.id}"
									class="btn btn-sm btn-info"> <i
										class="fa fa-pencil-square-o"></i>Edit
								</a> <a
									href="<%= request.getContextPath() %>/taskCategory/delete?id=${taskCategory.id}"
